...
class NodeRtmpSession {
  ...
  rtmpChunksCreate(packet) {
    ...
    // console.log('CREATED: ' + sha256(chunks));
    // console.log(chunks);
    // console.log(Buffer.from(chunks, 'hex').toString('utf8'));
    // console.log(header);
    return chunks;
  }
  ...
  rtmpVideoHandler() {
    ...
    // var body = Buffer.from(, 'hex').toString('utf8');
    // Logger.log(body);
    for (let playerId of this.players) {
      let playerSession = context.sessions.get(playerId);
      if (playerSession instanceof NodeRtmpSession) {
  ...
  rtmpDataHandler() {
    ...
    //console.log('RECEIVED: ' + sha256(rtmpChunks));
  }
